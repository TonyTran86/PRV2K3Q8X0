...
class ProcessReport:
    ...
    def get_dataframe(self):
        try:
            self.df_read = pd.read_excel(self.f_path, engine="openpyxl", index_col=False, dtype="unicode",
                                         na_values=['NA'])
            ProcessReport.print_out(f'INFO: get_dataframe | report file was read')

            return self.df_read

        except Exception as err:
            ProcessReport.print_out(f'ERROR: get_dataframe | {err.__class__.__name__} {err}', mode='error')
    ...
    def report_template_checking(self, debug=False):
        try:
            ...
            if debug:
                ProcessReport.print_out(f'INFO: col_count: {col_count} | col_check: {h_chk}')
            ...
            ProcessReport.print_out(f'get_template_checking error | {err.__class__.__name__} {err}', mode='error')
    ...
    def get_result_template(self):
        ...
        try:
            if self.template:
                ...
                ProcessReport.print_out(f'INFO: get_result_template | Result template was created')

            else:
                self.df_default = self.df_read
                ProcessReport.print_out(f'INFO: get_result_template | Report is default template')

        except Exception as err:
            ProcessReport.print_out(f'ERROR: get_result_template | {err.__class__.__name__} {err}', mode='error')
    ...
    def get_url_listing(self):
        try:
            ...
            ProcessReport.print_out(f'INFO: get_url_listing | URL listing was extracted')
            return self.list_zip
        except Exception as err:
            ProcessReport.print_out(f'ERROR: get_url_listing | {err.__class__.__name__} {err}', mode='error')
    ...
    def get_split_rows(self, arg):
        ...
        print(f'INFO: get_split_rows | Report {filename} : saved')
        logger.debug(f'INFO: get_split_rows | Report {filename} : saved')

    @staticmethod
    def get_split_rows_list(arg):
        data_range = len(arg)
        ...
        ProcessReport.print_out(f'INFO: get_split_rows_list | Data length: {data_range}')
        ProcessReport.print_out(f'INFO: get_split_rows_list | Total even steps: {total_even_step}')
        ProcessReport.print_out(f'INFO: get_split_rows_list | Even range: {even_range}')
        ProcessReport.print_out(f'INFO: get_split_rows_list | Remain: {remain_range}')
        ...
        for i in range(last_step):
            ...
            # print(x_range[i], x_range[i + 1])

        list_zip = zip(from_row, to_row)
        ProcessReport.print_out(f'INFO: get_split_rows_list | from_to_row was extracted')
    ...
    def save_split_row_files(self):  # save each split result file
        ...
        ProcessReport.print_out(f'INFO: save_split_row_files | Split files were saved')
    ...
    def save_result_file(self, file_name_suffix='_result.xlsx', save=True):  # Save result by route
        ...
        if save:
            ...
            ProcessReport.print_out(f'INFO: save_result_file | Result file was exported')
        else:
    ...
    def append_detection_data(self, detection_data, debug=False):
        ...
        if debug:
            df.to_excel('detection_append_data.xlsx', index=False)

        if self.df_default.empty:
            # Read result file
            self.df_default = pd.read_excel(self.result_file_path, engine="openpyxl", index_col=False, dtype="unicode",
                                            na_values=['NA'])
        ...
        ProcessReport.print_out(f'INFO: Detection result appending was completed')
